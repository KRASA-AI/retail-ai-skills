...
3. **Agentic traffic signals** — Any known automated-checkout sources (Google Agent Protocol, UCP-via-AI-Mode-or-Gemini, ChatGPT shopping, Claude / Anthropic agent commerce, a branded buying assistant, affiliate bots) and the share of orders they represent; merchant-attested agent IDs and signature-chain status; Web Bot Auth signature-verification rate at the edge; Visa Trusted Agent Protocol enrollment status; Mastercard Verifiable Intent record presence on Agent Pay flows; delegated-purchase token status (Stripe ACP / Shared Payment Token, SPT-on-BNPL coverage for Affirm / Klarna, Stripe Link for AI agents, Adyen Agent Token, Shopify Pay Agent, Visa MAAI, Mastercard Agent Pay, Amex Tokenized Agent Commerce); Machine Payments Protocol (MPP) endpoint exposure if the merchant accepts agent-paid microtransactions or recurring
...
3. **Rule tuning for agentic behavior** — Identify signals that are noisy for agents but valid for humans (short session, no mouse movement, instant form fill, headless user-agent) and propose replacement signals: delegated-purchase tokens (Stripe Shared Payment Token / SPT-on-BNPL for Affirm / Klarna / Adyen Agent Token / Shopify Pay Agent / Stripe Link for AI agents / Visa MAAI / Mastercard Agent Pay / Amex Tokenized Agent Commerce), merchant-attested agent IDs with signature-chain verification, Web Bot Auth signature-verification result at the edge (Cloudflare AI Crawl Control or equivalent), Visa Trusted Agent Protocol attestation outcome, Mastercard Verifiable Intent record (cryptographic pre-authorization scope: merchant, ceiling, time-window, item-class), passkey / 3DS2 device binding on the consumer's wallet, ship-to stability, BIN-level dispute history, and network-token cryptogram presence. For BNPL-on-SPT orders, layer in BNPL-specific signals (per-provider provider-decisioning code, agent-initiated installment first-payment success, BNPL-velocity per shopper) so an Affirm-or-Klarna agent flow does not get the typed-card guest-checkout rule set. For MPP-arriving orders (microtransaction or recurring metered), confirm the chargeback evidence trail still captures delivery confirmation, ship-to, and prior-undisputed-DPAN linkage even though the rail is machine-to-machine. Build the rule table as: signal → current threshold → proposed threshold → expected impact (approved-loss, false-decline, manual-review hours).
...
- **Agentic-checkout allowlist criteria** — by protocol-quality tier with the named protocol cited per row. 2026 tier ladder: (T1, highest) Stripe Link agent wallet or SPT with Verifiable Intent + passkey + 3DS2 + Web Bot Auth signature passing edge verification + Visa Trusted Agent Protocol or Mastercard Agent Pay attestation; (T2) delegated token (SPT / Adyen Agent Token / Shopify Pay Agent / Amex Tokenized Agent Commerce) + passkey + 3DS2 + MAAI signature-chain valid; (T3) attested agent ID + 3DS2 without delegated token; (T4) declared agent without attestation. SPT-on-BNPL flows route to T1 only when the BNPL provider returns an agent-initiated installment success on the first payment and the shopper-side passkey is on the issuer's wallet, otherwise T2. MPP-arriving orders default to T2 and require the merchant to add a per-merchant ceiling rule (max-MPP-spend-per-shopper-per-day) so a malformed agent loop cannot exhaust budget without surfacing.
...
- Correct card-scheme and dispute terminology (CE 3.0, FPT, MAAI, Agent Pay, ACP, DPAN, network token, RR codes, ARN, SCA, RBA, VDMP, ECP, EFM, Shared Payment Token, BNPL agent token, Stripe Link for AI agents, Web Bot Auth, Visa Trusted Agent Protocol, Mastercard Verifiable Intent, Machine Payments Protocol, Universal Commerce Protocol)
...
- A blanket block on agent traffic is the most expensive false-decline policy a retailer can adopt in 2026. The protocol-quality tier is the load-bearing distinction: a Stripe Link agent-wallet or SPT-with-Verifiable-Intent transaction with passkey + 3DS2 device binding and a passing Web Bot Auth edge signature is a stronger identity proof than a typed-card guest checkout, and the rubric must reflect that. The 2026 expansion of attestation primitives (Visa Trusted Agent Protocol on Web Bot Auth, Mastercard Verifiable Intent, Stripe Link for AI agents, SPT-on-BNPL, Machine Payments Protocol) means an unattested agent in 2026 is the *lowest*-trust shape, not a default — the merchant who has not staged a tier ladder is leaving conversion on the floor at every tier above T4.
