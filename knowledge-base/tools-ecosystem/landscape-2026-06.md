...
## Append: 2026-06-22 landscape monitor update

*Added by the 2026-06-22 monitor run (window: 2026-06-15 → 2026-06-22). This cycle the Shopify Spring '26 Edition cleared the version-bump gate; `agentic-commerce-readiness` was bumped v2.4 → v2.5. The notes below are concept-level grounding; no vendor copy is quoted or paraphrased verbatim.*

**§5 storefront / §2 protocols — Shopify Spring '26 Edition (June 17, 2026).** Shopify's flagship semi-annual release shipped 150+ updates centered on agentic commerce. The elements that change a merchant's readiness posture (and drove the `agentic-commerce-readiness` v2.5 bump):

- **Shopify Catalog goes default-on and multi-surface.** Eligible merchant listings are auto-syndicated *by default, with no setup* to multiple consumer AI surfaces — ChatGPT, Microsoft Copilot, Google AI Mode, Gemini, and the Shop app — with Shopify reporting that catalog-sourced agent searches convert materially better than answers built from scraped data. This is an escalation of the earlier "Shopify Catalog (MCP/REST) + Checkout Kit" note in §4: the readiness question shifts from "did the merchant build a catalog endpoint" to "did the merchant confirm which surfaces it is auto-enrolled on, verify the syndicated attribute set matches the on-page source of truth, and explicitly opt out of any surface it does not want."
- **Open public MCP agent endpoint (no approval gate).** As of June 17, any developer can register an agent profile in Shopify's Developer Dashboard and call the platform's public MCP endpoint with no approval gate — moving inbound third-party-agent access from gated to open-by-default on the Shopify surface. This is an agent-access-governance datapoint (ties to §3): an open endpoint raises the importance of verified-agent attestation and rate-limit headroom for distinguishing verified agents from disguised scrapers, and it reinforces the `agent-access-governance-policy` pipeline candidate.
- **Merchant-side agentic admin (Sidekick everywhere + Campaign Autopilot + AI sales associate + Sidekick App Extensions / Pulse).** Sidekick now spans the admin via typing or voice, ships an autonomous Campaign Autopilot, an AI sales associate, App Extensions with 15+ partners (Klaviyo, Loop, Smile, Judge.me, Yotpo, and others), and a Sidekick Pulse admin home. This is a strong *second* native-platform vendor for the **Agentic Merchant Admin Assistant** pipeline candidate alongside Shoplazza Athena (§5) — the two-vendor gate for that candidate is now met (see monitor pipeline notes).
- **Omnichannel / in-store returns, Shop Pay opens to any platform, POS v11, USDC cashback, Rollouts A/B testing.** Operational breadth; in-store returns for online orders is an omnichannel returns workflow adjacent to `return-policy-explainer` and `distributed-order-allocation`. No standalone skill gate met.

**§2 protocols/rails — protocol-translation layer and machine payments.** **Adyen Agentic** (announced June 16, 2026) positions itself as a universal translator across UCP, ACP, AP2, and Meta's AI checkout — a *second* named cross-protocol abstraction vendor alongside Visa Intelligent Commerce Connect (April 8, 2026). Concept to carry: as the protocol count grows, a merchant-side "single integration that speaks every agentic protocol" layer is consolidating into its own vendor category; `agentic-commerce-readiness` already grades per-protocol enrollment, so this is currently a checkout-stack option, not a new workflow. **Mastercard Agent Pay for Machines (AP4M)** extends Agent Pay to machine-to-machine programmatic transactions (microtransactions down to fractions of a cent, settled across cards, bank accounts, and stablecoins) — maturation of the machine-payments rail already named via MPP; relevant only to merchants exposing paid API surfaces to agents. **UCP-powered checkout retailer rollout** continues with the named set (Nike, Sephora, Target, Ulta, Walmart, Wayfair, plus Shopify merchants Fenty and Steve Madden) and announced geographic expansion (Canada, Australia, UK) — regional/retailer expansion of an already-mapped protocol, no version bump.

**Subscription-commerce context.** Subotiz launched an AI Agent Suite (Collect / Onboard / Merchant / Data / Email agents) and an MCP server for natural-language subscription-commerce management (June 2026) — single-vendor in the subscription-commerce niche; no retail-wide workflow gate met. Logged for completeness.

**GitHub / prompt-library sweep.** Generic multi-industry agent catalogs ("awesome-ai-agents-2026" variants) and reference e-commerce agent repos (e.g., A2A-protocol multi-agent demos with MCP product/inventory servers) only; no retail-specific workflow or technique not already covered. Nothing extracted (no anti-plagiarism exposure).
